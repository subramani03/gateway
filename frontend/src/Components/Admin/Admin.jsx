  ...
  return (
    <div className="my-4">
      ...
      {/* <div className="flex justify-end gap-3 mx-4">
        ...
        <fieldset className="fieldset">
          <legend className="fieldset-legend font-semibold mb-1">Filter</legend>
          <select name="event" onChange={filterFunction} className="select w-28 md:w-32 border border-primary rounded-lg">
            <option disabled>Select here</option>
            <option value="All">All</option>
            {formData?.events.map((e, i) => <option key={i} value={e.name}>{e.name}</option>)}
          </select>
        </fieldset>
      </div> */}
            ...
            <tr className="bg-zinc-950 text-primary">
              ...
              {Array.from({ length: maxParticipants }).map((_, i) => (
                <React.Fragment key={i}>
                  <th className="border p-2">Participant{i + 1} Name</th>
                  <th className="border p-2">Participant{i + 1} Roll No</th>
                </React.Fragment>
              ))}
      ...
      <div className="flex justify-center mt-6 gap-2 flex-wrap">
        ...
        {Array.from({ length: totalPages }).map((_, index) => (
          <button key={index} className={`px-3 py-1 border  border-primary rounded-full text-sm text-primary hover:bg-primary hover:text-white`} onClick={() => setCurrentPage(index + 1)}>{index + 1}</button>
        ))}
